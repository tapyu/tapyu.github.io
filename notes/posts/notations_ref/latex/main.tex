...
\begin{xltabular}{\textwidth}{XX}
    ...
    \(\delta_{m_l}^{(l)}(n), - \mathbf{e}^{(l)}(n) \odot \boldsymbol{\varphi}'(\mathbf{v}^{(l)}(n))\), \(\frac{\partial\mathscr{E}(n)}{\partial v_{m_l}^{(l)} (n)}\) &  Local gradient of the \(m_l\)th neuron of the \(l\)th layer. \\ \hline
