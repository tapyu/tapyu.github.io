...
\begin{align}
    ...
    \mathbf{H} = \left[ \dfrac{\partial^{2} f(\mathbf{x})}{\partial \mathbf{x} \partial \mathbf{x}^\top} \right]_{\textnormal{Num}} = \renewcommand{\arraystretch}{2.2}\begin{bmatrix}
...
\begin{align}
    ...
    \mathbf{H} = \left[ \dfrac{\partial^{2} f(\mathbf{x})}{\partial \mathbf{x}^2} \right]_{\textnormal{Den}} = \left[ \dfrac{\partial}{\partial \mathbf{x}} \left(\dfrac{\partial f(\mathbf{x})}{\partial \mathbf{x}}\right) \right]_{\textnormal{Den}} = \renewcommand{\arraystretch}{2.2}\begin{bmatrix}
        \dfrac{\partial^{2} f}{\partial x_1^2} & \dfrac{\partial^{2} f}{\partial x_1 \partial x_2} & \cdots & \dfrac{\partial^{2} f}{\partial x_1 \partial x_n} \\
        \dfrac{\partial^{2} f}{\partial x_2 \partial x_1} & \dfrac{\partial^{2} f}{\partial x_2^2} & \cdots & \dfrac{\partial^{2} f}{\partial x_2 \partial x_n} \\
        \vdots & \vdots & \ddots & \vdots \\
        \dfrac{\partial^{2} f}{\partial x_n \partial x_1} & \dfrac{\partial^{2} f}{\partial x_n \partial x_2} & \dots & \dfrac{\partial^{2} f}{\partial x_n^2}
    \end{bmatrix} \in \mathbb{R}^{n \times n},
\end{align}
where \(f: \mathbb{R}^{n} \rightarrow \mathbb{R}\) and \(\mathbf{x} \in \mathbb{R}^{n}\). The element \(\partial\mathbf{x}^2\) is merely a convention which means that \(\partial\mathbf{x}\) is repeated twice, in the very same way it is done with scalar differentiations. This notation is adopted by many authors \cite{diniz1997adaptive,haykinAdaptiveFilterTheory2002}. Note that it can be scaled to \(n\) consecutive derivatives without making the notation cumbersome (points to the denominator layout team).
